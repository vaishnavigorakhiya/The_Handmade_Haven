...
.form-input.is-valid,.form-select.is-valid,.form-textarea.is-valid{border-color:var(--teal)!important;}
...
.img-upload-area.dragover{border-color:var(--teal);background:#E8F8F5;}
...
.dbtn-cancel{background:white;}
.dbtn-cancel:hover{background:var(--dark);color:white;}
.dbtn-del{background:var(--coral);color:white;border-color:var(--coral);}
.dbtn-del:hover{background:#c0392b;}
...
<div class="admin-wrap">
  ...
  <div class="form-section">
    ...
    <p style="font-size:.88rem;color:var(--mid);font-weight:600;margin-bottom:18px;">Add your own product categories. They'll appear in all product forms automatically.</p>
    <div class="cat-grid">
        ...
        <div class="cat-chip" style="background:{{ $cat->color }}">
          ...
          <form action="{{ route('admin.categories.destroy',$cat->id) }}" method="POST" style="display:inline;">
            ...
            <button type="submit" class="cat-del-btn"
                    onclick="return confirm('Remove category \'{{ addslashes($cat->name) }}\'?\nProducts in this category will move to Custom.')"
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" id="addForm" novalidate>
      ...
      <div class="form-grid">
        ...
        <div class="form-group full">
          <label class="form-label">Product Image <span style="color:var(--teal);font-size:.72rem;font-weight:600;">(JPG/PNG/WEBP · max 2MB)</span></label>
          <div class="img-upload-area" id="aUpArea"
               ondragover="handleDragOver(event,'aUpArea')" ondragleave="handleDragLeave('aUpArea')"
               ondrop="handleDrop(event,'aImgInput','aUpArea','aPreviewWrap','aPlaceholder','aImgEl','aImgName')">
        ...
        <div class="form-group">
          <label class="form-label">Badge <span style="font-weight:600;color:var(--mid);">(optional)</span></label>
        ...
        <div class="form-group full">
          ...
          <textarea class="form-textarea" name="description" id="a_description" placeholder="Describe this piece — materials, size, care instructions…">{{ old('description') }}</textarea>
    ...
    <div class="modal-body">
      <form id="editForm" method="POST" enctype="multipart/form-data" action="" novalidate>
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            <label class="form-label">Product Name *</label>
            <input class="form-input" name="name" id="e_name" placeholder="Product name" />
            <div class="ferr" id="ee_name"></div>
          </div>
          <div class="form-group">
            <label class="form-label">Price ($) *</label>
            <input class="form-input" name="price" id="e_price" type="number" step="0.01" min="0" placeholder="0.00" />
            <div class="ferr" id="ee_price"></div>
          </div>
          <div class="form-group">
            <label class="form-label">Category *</label>
          ...
          <div class="form-group">
            <label class="form-label">Stock Qty *</label>
            <input class="form-input" name="stock" id="e_stock" type="number" min="0" placeholder="0" />
            <div class="ferr" id="ee_stock"></div>
          </div>
          <div class="form-divider"><span>Media &amp; Visual</span></div>
          <div class="form-group full">
            ...
            <div class="edit-img-row">
              ...
              <div id="eNoImg" style="font-size:.85rem;font-weight:700;color:var(--mid);align-self:center;">No image uploaded</div>
              <div style="flex:1;">
                ...
                <div class="img-upload-area" id="eUpArea" style="padding:14px;"
                     ondragover="handleDragOver(event,'eUpArea')" ondragleave="handleDragLeave('eUpArea')"
                     ondrop="handleDrop(event,'eImgInput','eUpArea','ePreviewWrap','ePlaceholder','eImgEl','eImgName')">
                  <input type="file" name="image" id="eImgInput" accept="image/*"
                         onchange="previewImg(event,'ePreviewWrap','ePlaceholder','eImgEl','eImgName')" />
                  <div id="ePlaceholder"><div class="upload-icon" style="font-size:1.4rem;">📸</div><div class="upload-title" style="font-size:.82rem;">Click or drag & drop</div><div class="upload-sub">JPG · PNG · WEBP</div></div>
          ...
          <div class="form-group">
            ...
            <input class="form-input" name="badge" id="e_badge" placeholder="Bestseller, New…" />
          </div>
          <div class="form-divider"><span>Description</span></div>
          <div class="form-group full">
            <label class="form-label">Product Description *</label>
            <textarea class="form-textarea" name="description" id="e_description" placeholder="Describe this piece…"></textarea>
            <div class="ferr" id="ee_description"></div>
          </div>
        </div>
        <div style="display:flex;gap:12px;margin-top:20px;flex-wrap:wrap;">
          <button type="button" class="form-submit" onclick="validateEdit()">💾 Save Changes</button>
          <button type="button" onclick="closeEdit()"
            style="padding:14px 28px;background:white;border:2.5px solid var(--dark);border-radius:50px;font-family:'Nunito',sans-serif;font-weight:800;cursor:pointer;box-shadow:3px 3px 0 var(--dark);">Cancel</button>
        </div>
      </form>
    </div>
  </div>
...
function filterTable(q){
  ...
  document.getElementById('tableEmpty').style.display=v===0?'block':'none';
}

/* IMAGE HELPERS */
function previewImg(ev,wId,phId,iId,nId){const f=ev.target.files[0];if(f)showFile(f,wId,phId,iId,nId);}
function showFile(f,wId,phId,iId,nId){
  const r=new FileReader();
  r.onload=e=>{
    document.getElementById(iId).src=e.target.result;
    document.getElementById(nId).textContent=f.name+' ('+(f.size/1024).toFixed(0)+' KB)';
...
function handleDragOver(e,aId){e.preventDefault();document.getElementById(aId).classList.add('dragover');}
function handleDragLeave(aId){document.getElementById(aId).classList.remove('dragover');}
function handleDrop(e,iId,aId,wId,phId,imgId,nId){
  e.preventDefault();document.getElementById(aId).classList.remove('dragover');
  const f=e.dataTransfer.files[0];
  if(f&&f.type.startsWith('image/')){
    const dt=new DataTransfer();dt.items.add(f);
    document.getElementById(iId).files=dt.files;
    showFile(f,wId,phId,imgId,nId);
  }
}
...
function validateAdd(){
  let ok=true;
  ['a_name','a_price','a_category','a_stock','a_description'].forEach(id=>{
    const el=document.getElementById(id);if(el){el.classList.remove('is-invalid','is-valid');}
    const em=document.getElementById('ae_'+id.substring(2));if(em)em.textContent='';
  });
  const n=document.getElementById('a_name').value.trim();
  const p=document.getElementById('a_price').value.trim();
  const c=document.getElementById('a_category').value;
  const s=document.getElementById('a_stock').value.trim();
  const d=document.getElementById('a_description').value.trim();
  if(!n){setE('a_name','ae_name','Product name is required.');ok=false;}
  else if(n.length<3){setE('a_name','ae_name','Name must be at least 3 characters.');ok=false;}
  else clrE('a_name','ae_name');
  if(!p){setE('a_price','ae_price','Price is required.');ok=false;}
  else if(isNaN(p)||parseFloat(p)<0){setE('a_price','ae_price','Enter a valid price.');ok=false;}
  else clrE('a_price','ae_price');
  if(!c){setE('a_category','ae_category','Please select a category.');ok=false;}
  else clrE('a_category','ae_category');
  if(s===''){setE('a_stock','ae_stock','Stock is required.');ok=false;}
  else if(isNaN(s)||parseInt(s)<0){setE('a_stock','ae_stock','Stock must be 0 or more.');ok=false;}
  else clrE('a_stock','ae_stock');
  if(!d){setE('a_description','ae_description','Description is required.');ok=false;}
  else if(d.length<10){setE('a_description','ae_description','At least 10 characters required.');ok=false;}
  else clrE('a_description','ae_description');
...
function validateEdit(){
  let ok=true;
  ['e_name','e_price','e_category','e_stock','e_description'].forEach(id=>{
    const el=document.getElementById(id);if(el){el.classList.remove('is-invalid','is-valid');}
    const em=document.getElementById('ee_'+id.substring(2));if(em)em.textContent='';
  });
  const n=document.getElementById('e_name').value.trim();
  const p=document.getElementById('e_price').value.trim();
  const c=document.getElementById('e_category').value;
  const s=document.getElementById('e_stock').value.trim();
  const d=document.getElementById('e_description').value.trim();
  if(!n){setE('e_name','ee_name','Product name is required.');ok=false;}
  else if(n.length<3){setE('e_name','ee_name','At least 3 characters.');ok=false;}
  else clrE('e_name','ee_name');
  if(!p){setE('e_price','ee_price','Price is required.');ok=false;}
  else if(isNaN(p)||parseFloat(p)<0){setE('e_price','ee_price','Enter a valid price.');ok=false;}
  else clrE('e_price','ee_price');
  if(!c){setE('e_category','ee_category','Please select a category.');ok=false;}
  else clrE('e_category','ee_category');
  if(s===''){setE('e_stock','ee_stock','Stock is required.');ok=false;}
  else if(isNaN(s)||parseInt(s)<0){setE('e_stock','ee_stock','Stock must be 0 or more.');ok=false;}
  else clrE('e_stock','ee_stock');
  if(!d){setE('e_description','ee_description','Description is required.');ok=false;}
  else if(d.length<10){setE('e_description','ee_description','At least 10 characters.');ok=false;}
  else clrE('e_description','ee_description');
...
function submitCat(){
  ...
  err.textContent='';
  if(!v){err.textContent='Category name is required.';document.getElementById('catInput').focus();return;}
  if(v.length<2){err.textContent='Name must be at least 2 characters.';document.getElementById('catInput').focus();return;}
...
function openEdit(id,p){
  ...
  const img=document.getElementById('eCurImg');
  const noImg=document.getElementById('eNoImg');
  if(p.image){img.src='/storage/'+p.image;img.style.display='block';noImg.style.display='none';}
