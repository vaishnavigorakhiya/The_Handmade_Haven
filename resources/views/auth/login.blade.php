    ...
    <div class="lm-right">
      ...
      <div class="lm-step" id="lmStep1">
        ...
        <div class="lm-field-wrap">
          <input type="text" id="lmIdentifier" class="lm-input"
                 ...
                 onkeydown="if(event.key==='Enter') lmSubmitStep1()" />
      ...
      <div class="lm-step" id="lmStep2OTP" style="display:none;">
        ...
        <div class="lm-otp-row" id="lmOtpBoxes">
          <input class="lm-otp-box" type="text" maxlength="1" id="lmOtp0" oninput="lmOtpIn(this,0)" onkeydown="lmOtpKey(this,0,event)" inputmode="numeric" />
          <input class="lm-otp-box" type="text" maxlength="1" id="lmOtp1" oninput="lmOtpIn(this,1)" onkeydown="lmOtpKey(this,1,event)" inputmode="numeric" />
          <input class="lm-otp-box" type="text" maxlength="1" id="lmOtp2" oninput="lmOtpIn(this,2)" onkeydown="lmOtpKey(this,2,event)" inputmode="numeric" />
          <input class="lm-otp-box" type="text" maxlength="1" id="lmOtp3" oninput="lmOtpIn(this,3)" onkeydown="lmOtpKey(this,3,event)" inputmode="numeric" />
          <input class="lm-otp-box" type="text" maxlength="1" id="lmOtp4" oninput="lmOtpIn(this,4)" onkeydown="lmOtpKey(this,4,event)" inputmode="numeric" />
          <input class="lm-otp-box" type="text" maxlength="1" id="lmOtp5" oninput="lmOtpIn(this,5)" onkeydown="lmOtpKey(this,5,event)" inputmode="numeric" />
        </div>
      ...
      {{-- STEP 2b: Password (existing email users — LOGIN ONLY) --}}
      <div class="lm-step" id="lmStep2Pass" style="display:none;">
        <div class="lm-step-tag">Step 2 of 2</div>
        <h3 class="lm-step-title">Enter Password 🔐</h3>
        <p class="lm-step-sub">Welcome back! Logging in as <strong id="lmEmailLabel"></strong></p>
        <div class="lm-field-wrap">
          <div class="lm-pass-wrap">
            <input type="password" id="lmPassword" class="lm-input"
                   placeholder="Your password"
                   autocomplete="current-password"
                   onkeydown="if(event.key==='Enter') lmSubmitPass()" />
      ...
      {{-- Register (new email only — shown ONLY when email does NOT exist) --}}
      <div class="lm-step" id="lmStep2Reg" style="display:none;">
        ...
        <p class="lm-step-sub">New here! Creating account for <strong id="lmRegEmailLabel"></strong></p>
        <div class="lm-field-wrap">
        ...
        <div class="lm-field-wrap">
          ...
          <div class="lm-pass-wrap">
            <input type="password" id="lmRegPass" class="lm-input"
                   placeholder="Minimum 8 characters"
                   autocomplete="new-password"
                   oninput="lmStrength(this.value)" />
        ...
        <div class="lm-field-wrap">
          ...
          <div class="lm-pass-wrap">
            <input type="password" id="lmRegConfirm" class="lm-input"
        ...
        <p class="lm-legal" style="margin-top:10px;">
          Already have an account? <a href="#" onclick="lmBack();return false;">Login instead</a>
        </p>
      </div>
...
window.closeLMIfOutside = function(e) {
...
function clrErr(id, eid) {
  const el = document.getElementById(id);
  const em = document.getElementById(eid);
  if (el) { el.classList.remove('err'); el.classList.add('ok'); }
...
window.lmSubmitStep1 = function() {
  ...
  if (!val) {
    setErr('lmIdentifier','lmIdentifierErr','Please enter your email or phone number.');
  ...
  if (!isPhone && !isEmail) {
    setErr('lmIdentifier','lmIdentifierErr','Please enter a valid email or 10-digit phone number.');
    document.getElementById('lmIdentifier').focus();
    return;
  }
  ...
  .then(data => {
    ...
    if (data.step === 'otp') {
      ...
      setErr('lmIdentifier','lmIdentifierErr','');
    }
  })
  .catch(() => {
    hideLoad();
    document.getElementById('lmStep1Err').textContent = 'Something went wrong. Please try again.';
  });
};
...
window.lmSubmitPass = function() {
  ...
  document.getElementById('lmPassErr').textContent = '';
  clrErr('lmPassword','lmPasswordErr');

  if (!pw) {
    setErr('lmPassword','lmPasswordErr','Password cannot be empty.');
  ...
  fetch('/auth/password', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF(), 'Accept': 'application/json' },
    body: JSON.stringify({ password: pw, remember: document.getElementById('lmRemember').checked })
  })
  .then(r => r.json())
  .then(data => {
    hideLoad();
    if (data.redirect) {
      ...
      document.getElementById('lmPassErr').textContent = data.error;
      setErr('lmPassword','lmPasswordErr','');
    }
  })
...
window.lmSubmitOtp = function() {
  ...
  const err = document.getElementById('lmOtpErr');
  err.textContent = '';

  if (otp.length < 6) {
    err.textContent = 'Please enter all 6 digits.';
    [0,1,2,3,4,5].forEach(i => {
      const b = document.getElementById('lmOtp'+i);
      b.classList.add('shake');
      setTimeout(() => b.classList.remove('shake'), 500);
    });
    document.getElementById('lmOtp0').focus();
    return;
  }
...
window.lmResend = function() {
  ...
  .then(data => {
    ...
    e.textContent = data.message || 'OTP resent!';
...
window.lmOtpIn = function(el, i) {
  ...
  if (el.value) {
    ...
    if (i < 5) document.getElementById('lmOtp'+(i+1)).focus();
    const all = [0,1,2,3,4,5].every(j => document.getElementById('lmOtp'+j).value);
    if (all) setTimeout(lmSubmitOtp, 200);
  } else {
...
window.lmOtpKey = function(el, i, e) {
  if (e.key === 'Backspace' && !el.value && i > 0)
    document.getElementById('lmOtp'+(i-1)).focus();
...
window.lmSubmitReg = function() {
  ...
  fetch('/auth/register', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF(), 'Accept': 'application/json' },
    body: JSON.stringify({ name: nm, email: em, password: pw, password_confirmation: cf })
  })
  .then(r => r.json())
  .then(data => {
    hideLoad();
    if (data.redirect) {
      ...
      document.getElementById('lmRegErr').textContent = data.error;
    } else if (data.errors) {
      document.getElementById('lmRegErr').textContent = Object.values(data.errors).flat()[0] || 'Registration failed.';
    }
  })
