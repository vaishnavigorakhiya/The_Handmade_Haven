...
<style>
  ...
  .dash-hero {
    background: linear-gradient(135deg, #FFE8D6, #FFD6F0, #E8F8F5);
  ...
  .dash-avatar {
    ...
    font-size: 2rem; box-shadow: 4px 4px 0 var(--dark); flex-shrink: 0;
  }
  ...
  /* Coupon */
  .coupon-box {
    background: linear-gradient(135deg, #FFE8D6, #FFD6F0);
    border: 2.5px dashed var(--dark); border-radius: 14px;
  ...
  .orders-table td { padding: 12px 0; border-top: 2px solid var(--cream); font-weight: 600; font-size: 0.9rem; }
  .order-status { padding: 3px 10px; border-radius: 50px; font-size: 0.75rem; font-weight: 800; border: 2px solid var(--dark); }
  ...
  <div class="dash-page">
    ...
    <div class="dash-grid">
      <div class="dash-card">
        ...
        <div style="font-size:0.8rem;font-weight:700;color:var(--mid);margin-top:12px;">
          🏷 10% off your first order · Valid for 30 days
        </div>
      </div>
      ...
      <div class="dash-card">
        ...
        <form method="POST" action="{{ route('user.profile.update') }}">
          ...
          <label style="font-size:0.8rem;font-weight:800;color:var(--mid);text-transform:uppercase;letter-spacing:0.06em;display:block;margin-bottom:6px;">
            {{ $user->phone ? 'Phone' : 'Email' }}
          </label>
          <input class="profile-input" value="{{ $user->phone ?? $user->email }}" disabled
    ...
    <div class="dash-card" style="margin-bottom:24px;">
      ...
      @else
        <table class="orders-table">
          <thead><tr>
            <th>Order #</th><th>Date</th><th>Amount</th><th>Status</th>
          </tr></thead>
          <tbody>
